...
Therefore the function the node implements is evaluated at the minimal timestamp of the buffered events, this timestamp is called the \emph{evaluation timestamp}.
...
The inverse of that statement shows shows why this is important: a function is never evaluated at a timestamp where no event is present.

Nodes having signals as inputs always have to remember the last occured change of them in their state.
When such a node is scheduled, the function will work either on the remembered value if no new change is present at the evaluation timestamp for the signal.
If a new change is present at the evaluation timestamp that will be used and the state of the node has to be updated to remember the new change.
...
Complete functions will consume one event from every input and produce at least one event at every timestamp they are evaluated.
...
Basically they are also incomplete functions, but they have to buffer more events.
TODO
...
Each node has a state, which contains arbitray information, e.g.\ a counter for a \emph{CountNode}, its input queues holding the non-processed events and, if they have signals as inputs, the last changes of them.
...
Everytime a node \(n\) is scheduled it changes its own node state and the node state of its direct children \(N_c\):

\begin{itemize}
  \item Check for the next timestamp \(t\) where a new output can be created
...
A transition describes what happens when the evaluation engine switches state by performing a step, meaning that a node was scheduled to perform its computation:
Events from the the inputs of a node are removed, one ore more output events can be generated and distributed and the progress of the output of the node is changed.
...
Note that all transitions, which produces events have to consume at least one event (therefore no events can be created from nowhere) and that it's possible that no event was produced based on the consumed events (think of a \emph{FilterNode}).
Furthermore it's possible to create multiple events in one transition.
