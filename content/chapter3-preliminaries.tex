...
Timing functions can produce events at every timestamp and produce multiple events per input event.
...
\begin{longtable}{lp{3cm}lp{6cm}}
  ...
  \(\mathit{delayEventByCount}\)        & Events & Events  & Delays each event of an eventstream by a specified number of events. For example it can be used to change the timestamp of each event to the timestamp of the next event received.\\
  ...
  \(\mathit{inPast}\)                   & Events & Signal  & Emits a boolean signal which holds the value \emph{true} whenever an event happened on the input before in a time specified.\\
