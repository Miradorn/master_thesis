...
Interestingly, this architecture enables the enforcement of greedy schedules, which are described in \Cref{def:greedy_schedule}, while the Erlang runtime itself only guarantees a fair scheduling.
...
It is easy to implement such an instrumentation with the provided C++ \gls{api} from \gls{llvm} to implement compiler passes that can analyse and transform \gls{ir} code.
