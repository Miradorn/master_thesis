...
\begin{proof}
  ...
  Therefore \(a\) is the enabled node with the lowest label at step \(i + j\) and therefore will be scheduled.

  If \(a\) is a source it will be enabled at one step, because evaluation engines work over finite traces, therefore at some point no new external events will be present.
  Even if \(a\) was never scheduled upto that point, now all sources will consume the remaining, buffered external events and become disabled.
  Therefore \(a\) at one point will become the source with the smallest label and be scheduled.
\end{proof}
